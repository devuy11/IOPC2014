...
int nodes,edges,to[Edges],fin[Edges],next[Edges],cop[Nodes],dist[Nodes],cap[Edges],flow[Edges],Q[Nodes],answer,M,V;

int src,snk,S,T,parcels,clean[101][101];

int N,J,C,ind1,ind2;
int X[100],A[100],B[100],P[100];
...
void flyod()
{
	...
	for(int k=0;k<N;k++){
		for(int i=0;i<N;i++){
			for(int j=0;j<N;j++){
				if(clean[i][j]>clean[i][k]+clean[k][j])
					clean[i][j]=clean[i][k]+clean[k][j];
			}
		}
	}
}
void graph()
{
	...
	edges=0;
	sum=0;
	//scanf("%d%d%d",&N,&J,&C);		//N is the variety of goods, C is the cost per area
	...
	for(int i=0;i<N;i++){
		for(int j=0;j<N;j++){
			if(clean[i][j]!=INF)
			if(B[i]+clean[i][j]*V<=A[j]){
				assert(clean[i][j]<=100000000);
